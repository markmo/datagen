...
function create_account_holdings(customer_key::ASCIIString)
    ...
    DataFrame(
        customer_sk                     = rep(customer_key, num_accounts),
...
#     account_type_code = get_account_type(account_key)
    ...
    DataFrame(
        account_sk                      = rep(account_key, days),
...
#     account_type_code = get_account_type(account_key)
    ...
    if num_transactions > 0
        DataFrame(
            ...
            account_sk                    = rep(account_key, num_transactions)
            )
    else
        nothing
    end
...
function create_channel_usage(customer_key::ASCIIString, enddate::ASCIIString, days::Int)
    ...
    DataFrame(
        customer_sk                     = rep(customer_key, days),
